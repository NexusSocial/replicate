...
pub async fn launch_webtransport_server(args: Args, _im: Arc<InstanceManager>) -> Result<()> {
    ...
    let refresh_cert_fut = async {
        ...
        loop {
            ...
            svr_ctx_l.cert = wtransport::Certificate::self_signed(svr_ctx_l.san.iter()).into();
...
async fn handle_connection(svr_ctx: ServerCtx, incoming: IncomingSession) -> Result<()> {
        ...
        match request {
            ...
            Sb::InstanceUrlRequest { id } => {
                let url = {
                    ...
                    let domain_name = svr_ctx_l.san.first().expect("should have domain name");
